...
если
для каждого числа $\varvarepsilon>0$
найдется такое натуральное число (номер) $N(\varvarepsilon)\in \mathbb{N}$,
что $|a_n - a|<\varvarepsilon$ при каждом $n>N(\varvarepsilon)$.
То же самое утверждение можно переписать в кванторах $\forall-\exists$
следующим образом:
$$
\forall\varvarepsilon>0\, \exists N(\varvarepsilon)\in \mathbb{N}\colon \forall n>N(\varvarepsilon)\ |a_n-a|<\varvarepsilon.
$$
...
$$
поэтому при $N(\varvarepsilon)=[\frac{1}{\varvarepsilon}]+1>\frac{1}{\varvarepsilon}$ выполнено
$|a_n-a|<\varvarepsilon$.
...
\begin{proof}
Действительно, если $a\ne b$, то $|a-b|=\varvarepsilon_0>0$.
Но по определению найдется номер $N_1$,
для которго $|a_n-a|<\frac{\varvarepsilon_0}{2}$ при $n>N_1$
и найдется номер $N_2$, для которого $|a_n-b|<\frac{\varvarepsilon_0}{2}$
при $n>N_2$.
Тогда при $n>\max\{N_1, N_2\}$ выполено
$$
\varvarepsilon_0 = |a-b| = |a - a_n + a_n - b|\le |a-a_n| + |a_n-b|<\varvarepsilon_0.
$$
Противоречие.
\end{proof}
...
\begin{proof}
Пусть $\varvarepsilon>0$ --- произвольное число.
Тогда найдется номер $N_1$, для которого $|a_n-a|<\varvarepsilon$,
и найдется номер $N_2$, для которого $|b_n-b|<\varvarepsilon$.
...
\le
|\alpha| |a_n-a| + |\beta| |b_n-b|< (|\alpha| + |\beta|)\varvarepsilon.
$$
...
при $n>N=\max\{N_1, N_2\}$
выполнено
$|a_nb_n - ab|\le (M + |a|)\varvarepsilon.$
...
$$
\Bigl|\frac{1}{b_n}-\frac{1}{b}\Bigr| = \frac{|b_n-b|}{|b_n||b|}\le \frac{2}{|b|^2}\cdot \varvarepsilon.
$$
\end{proof}
...
$\forall \varvarepsilon > 0: N(\varvarepsilon) \in \mathbb{N} \ \forall n > N(\varvarepsilon): |a_n - a| < \varvarepsilon \\ \text{Возьмем } \varvarepsilon = \frac{|a|}{2} > 0: \exists N \ \forall n > N: |a_n - a| < \frac{|a|}{2} \\ |a_n| = |a_n + a - a| \geq |a| - |a_n - a| > \frac{|a|}{2}$
\end{proof}
...
% \end{statement}
\begin{proof}
Предположим $a - b = \varvarepsilon_0 > 0 \Rightarrow\\\Rightarrow \exists N_1,\, N_2: \left|a_n - a\right| < \frac{\varvarepsilon_0}{2}\;\forall\, n>N_1,\; \left|b_n - b\right| < \frac{\varvarepsilon_0}{2}\; \forall\, n>N_2 \Rightarrow\\\Rightarrow \varepsilon_0 = a - b = a - a_n + a_n - b + b_n - b_n \le a - a_n + b_n - b < \varvarepsilon_0$ -- противоречие. 
\end{proof}
...
\end{lemma}
\begin{proof}
По определению $\forall\varvarepsilon\;\exists\, N_1\in\mathbb{N},\,N_2\in\mathbb{N}: \left|a - a_n\right| < \varvarepsilon\,\forall n > N_1,\; \left|b - b_m\right| < \varvarepsilon\, \forall m > N_2 \Rightarrow \forall k > \max\{N, N_1, N_2\}: a - \varvarepsilon < a_k \le c_k \le b_k < a + \varvarepsilon \Rightarrow \lim\limits_{n\to\infty} c_n = a$
\end{proof}
...
Но $\lim\limits_{n\to\infty} b_n - a_n = 0 \Rightarrow \forall\varvarepsilon>0\;\exists N(\varvarepsilon)\in\mathbb{N}:\forall n\ge N(\varvarepsilon) \left|0 - b_n + a_n\right| < \varvarepsilon$. \\
Пусть $\varvarepsilon = c' - c$, тогда $\left|a_n - b_n\right| < c' - c \Rightarrow b_n - a_n < c' - c$ -- противоречие. 
\end{proof}
...
Тогда для каждого $\varvarepsilon>0$ найдется номер $N\in \mathbb{N}$, для которого
$M-\varvarepsilon< a_N$ (иначе $M-\varvarepsilon$ --- верхняя грань, чего не может быть).
В силу того, что последовательность неубывающая, при каждом $n>N$ выполнено
$$M-\varvarepsilon< a_N\le a_n\le M< M+\varvarepsilon.$$
...
Говорят, что последовательность $\{a_n\}^\infty_{n=1}$ фундаментальна (или является последовательностью Коши), если для каждого числа $\varvarepsilon > 0$ найдется такое натуральное число (номер) $N(\varvarepsilon) \in \mathds{N}$, что $|a_n - a_m| < \varvarepsilon$ при каждых $n, m > N(\varvarepsilon)$. То же самое утверждение можно переписать в кванторах $\forall - \exists$ следующим образом: $$\forall \varvarepsilon > 0 \exists N(\varvarepsilon) \in \mathds{N}: \forall n, m > N(\varvarepsilon) \ |a_n - a_m| < \varvarepsilon$$ \\ 
\textbf{Пример.} \begin{itemize}
    \item[1)]Последоввательность $a_n =\frac{1}{n}$  фундаментальная. Действительно $$\left|\frac{1}{n} - \frac{1}{m}\right| \leqslant max\left\{\frac{1}{n}, \frac{1}{m}\right\},$$ поэтому при $N(\varvarepsilon) = \left[\frac{1}{\varvarepsilon}\right] + 1 > \frac{1}{\varvarepsilon} \text{ выполнено } |a_n - a_m| < \varvarepsilon$
    \item[2)] Последовательность $a_n = (-1)^n$ не фундаментальная. Действительно, если мы возьмем  $\varvarepsilon = 1$, то, какой бы ни был номер $N(\varvarepsilon)$, для произвольного $n > N(\varvarepsilon)$ выполнено $|a_n - a_{n+1}| = 2 > 1.$
\end{itemize} 
\textbf{Критерий Коши}. $\{a_n\}_{n=1}^\infty$ - сх-ся $\iff$ $\{a_n\}_{n=1}^\infty$ - посл. Коши
\begin{proof}
$\Longrightarrow$ Пусть $\varvarepsilon > 0$ По определению сходящейся последовательности найдется такой номер $N \in \mathds{N}$, что $|a_n - a| < \frac{\varvarepsilon}{2}$ при $n > N$, где $ \lim_{n\to\infty} a_n = a$. Тогда при $n, m > N$ выполнено $$|a_n - a_m| = |a_n - a_m + a - a| \leqslant |a_n - a| + |a_m - a| < \varvarepsilon$$ \\ $\Longleftarrow$(План: 1. Ограничена 2. предел по т. Больцано 3. $a = \lim_{n\to\infty}a_n$) \begin{itemize}
    \item[1.] Заметим, что последовательность $\{a_n\}_{n=1}^\infty$ ограничена. $\varvarepsilon = 1 \ \exists N: \forall n, m > N: |a_n - a_m| < 1$ (из условия). Отсюда $|a_n| = |a_n + a_{N+1} - a_{N+1}| \leqslant |a_n - a_{N+1}| + |a_{N+1}| < 1 + |a_{N+1}|$, при $n>N$. Значит, $$|a_n| < M = max\{1+|a_{N+1}|, |a_1|, ..., |a_N|\}.$$
    ...
    \item[3.] $\forall \varvarepsilon > 0\ \exists k_0: k > k_0\ |a_{n_{k}} - a| < \varvarepsilon.$ Кроме того, в силу фундаментальности найдется номер N, для которого $|a_n - a_m| < \varvarepsilon$ при $n, m > N$. Пусть $k$ выбрано так, что $k > k_0$ и $n_k > N$, тогда при каждом $n > N$ выполнено, что $$|a_n - a| = |a_n + a_{n_{k}} - a_{n_{k}} - a| < \underset{<\varvarepsilon}{|a_n - a_{n_k}|} + \underset{<\varvarepsilon}{|a- a_{n_k}|} < 2\varvarepsilon$$
\end{itemize}
\end{proof}
\subsection{Расходимость последовательности $a_n = \sum^n_{k=1}\frac{1}{k}$} Проверим отридцание фундаментальности $$\exists \varvarepsilon > 0 \ \forall N: \exists \underset{n > m}{n,m > N}:|a_n - a_m| \geqslant \varvarepsilon$$ \\ $|a_n - a_m| = \frac{1}{m+1} + \frac{1}{m+2}+...+\frac{1}{n} 
\geqslant \frac{n-m}{n}= 1 - \frac{m}{n}$ Для $\varvarepsilon = \frac{1}{2}, n = 2m, m > N \Longrightarrow |a_n - a_m| \geqslant \frac{1}{2} \Longrightarrow$ не выполнено условие Коши $\Longrightarrow$ последовательность расходится
\subsection{Вычисление $\sqrt{2}$ с помощью рекуррентной формулы $a_{n+1} = 1 + \frac{1}{1+a_n}, a_1 = 1$, обоснование сходимости} Заметим, что $a_n \geqslant 1$ и $$|a_{n+1} - a_n| = \left|\frac{1}{1+a_n} - \frac{1}{1+a_{n-1}} \right| = \frac{|a_{n-1} - a_n|}{(1+a_n)(1+a_{n+1})} \leqslant \frac{1}{4}|a_{n-1} - a_n| \leqslant \left(\frac{1}{4}\right)^{n-1}(a_2 - a_1) = \left(\frac{1}{4}\right)^{n-1} \frac{1}{2}$$ Отсюда при $m > n:$ $$|a_m - a_n| \leqslant |a_m - a_{m-1}| +...+ |a_{n+1} - a_n| \leqslant \frac{1}{2}\left(\left(\frac{1}{4}\right)^{m-2} + ... + \left(\frac{1}{4}\right)^{n-1}\right) = \frac{1}{2}\left(\frac{1}{4}\right)^{n-1}\left(\frac{1 - \left(\frac{1}{4}\right)^{m-n}}{1 - \frac{1}{4}}\right) \leqslant$$\newline$$\leqslant \frac{1}{2}\left(\frac{1}{4}\right)^{n-1}\frac{1}{\frac{3}{4}} = \frac{2}{3}\left(\frac{1}{4}\right)^{n-1} = \frac{8}{3}\left(\frac{1}{4}\right)^n$$ Т.к. $\left(\frac{1}{4}\right)^n \rightarrow 0: \forall \varvarepsilon > 0 \ \exists N: n> N \ \left(\frac{1}{4}\right)^n < \varvarepsilon$. Тем самым, для последовательности $\{a_n\}_{n=1}^\infty$ выполнен критерий Коши, а значит существует $A = \lim_{n\to\infty} a_n \Longrightarrow \lim_{n\to\infty} a_n = A = \lim_{n\to\infty} a_{n+1} = 1 + \frac{1}{1 + A} \\ A(A+1) = A + 1 + 1 \iff A^2 = 2 \iff A = \sqrt{2}$ т.к. $a_n \geqslant 0$
...
$$\forall \varvarepsilon > 0 \ \exists N \in \mathbb{N} : \forall n > m > N \to \left|\displaystyle\sum_{k = m + 1}^n a_k\right| = |S_n - S_m| < \varvarepsilon$$

\subsection{Необходимое условие сходимости числового ряда}
Если числовой ряд сходится, то $a_k \to 0$ при $k \to \infty $
\begin{proof}
Из критерия коши следует, что: $$\forall \varvarepsilon > 0 \ \exists N \in \mathbb{N}: \forall n >  N + 1 \to |a_n| = |S_n - S_{n-1}| < \varvarepsilon$$
\end{proof}
...
$$\forall \varvarepsilon > 0 \ \exists N \in \mathbb{N}: \forall n > m > N \to \displaystyle\sum_{k = m + 1}^{n}|a_k| < \varvarepsilon$$
...
$\varvarepsilon$-окрестность точки $a \in\mathbb{R}$ называется множество $B_\varvarepsilon(a) := \{x\in\mathbb{R}: |x-a| < \varvarepsilon\} = (a-\varvarepsilon, a +\varvarepsilon).$

\textbf{Определение 2}.

Проколотая $\varvarepsilon$-окрестность точки $a \in\mathbb{R}$ называется множество $B'_\varvarepsilon(a) := B_\varvarepsilon \backslash \{a\}$.

\textbf{Определение 3}.

Множество $U\subset\mathbb{R}$ называется \textbf{открытым}, если для любой $a\in U$ найдётся такое $\varvarepsilon > 0$, что $B_\varvarepsilon \subset U$.
...
\begin{itemize}
    \item[1.1] Пусть $U = \bigcup\limits_{\alpha\in A}U_\alpha$, причём все $U_\alpha - $ открытые множества. Если $a\in U$, тогда найдётся такой индекс $\alpha$, что $a\in U_\alpha$. По определению найдётся такое $\varvarepsilon > 0$, что $B_\varvarepsilon(a) \subset U_\alpha$. Значит, по определению операции объединения, $B_\varvarepsilon(a)\subset U$. Т.е. $U - $ открытое множество.
    \item[1.2] Пусть $U = \bigcap\limits_{j=1}^NU_j$, причём все $U_j - $ открытые множества. Если $a\in U$, то для каждого \newline$j\in \{1,...,N\}$ найдётся такое число $\varvarepsilon_j>0$, что $B_{\varvarepsilon_{j}}(a) \subset U_j$. Пусть $\varvarepsilon := \min\{\varvarepsilon_1,...,\varvarepsilon_N\}>0$. Тогда $B_\varvarepsilon(a)\subset B_{\varvarepsilon_j}(a) \subset U_j$ при каждом $j \in \{1,...,N\}$. Значит, $B_\varvarepsilon(a) \subset U$ и $U - $ открытое множество.
...
Точка $a\in\mathbb{R}$ называется \textbf{внутренней} точкой множества $M$, если она входит в это множество $M$ с некоторой своей окрестностью \underline{полностью} (т.е. $\exists\varvarepsilon > 0$ : $B_\varvarepsilon(a)\subset M$).

\textbf{Определение 6.}

Точка $a\in\mathbb{R}$ называется \textbf{предельной} точкой множества $M$, если каждая её проколотая окрестность имеет непустое пересечение с множеством $M$ (т.е. $\forall\varvarepsilon > 0: B'_\varvarepsilon(a)\cap M\neq \o$).

\textbf{Определение 7.}

Точка $a\in\mathbb{R}$ называется \textbf{граничной} точкой множества $M$, если каждая её окрестность имеет непустое пересечение как с множеством $M$, так и с его дополнением (т.е. $\forall\varvarepsilon > 0: B_\varvarepsilon(a)\cap M\neq \o$ и $B_\varvarepsilon(a)\cap(\mathbb{R}\backslash M)\neq\o$).
...
Наоборот (если есть сходящаяся к $a$ последовательность, то $a - $ предельная точка для $M$), если $a_n\in M\backslash\{a\},$ то каждого $\varvarepsilon>0$ найдётся такой номер $N$, что $|a_n - a|<\varvarepsilon$ при $n>N$. Таким образом, $a_{N+1}\in B'_\varvarepsilon(a)\cap M$.
\end{proof}
...
\begin{itemize}
    \item[$1) \Rightarrow2)$]\textit{(Если $V$ - замкнутое множество, то оно содержит все свои граничные точки)}: \newline Пусть $a$ граничная точка для $V$, для которой выполнено, что $a\not\in V$, то $a\in\mathbb{R}\backslash V$ - открытое множество. Это значит, что найдётся такое $\varvarepsilon > 0$, что $B_\varepsilon(a)\subset \mathbb{R}\backslash V$ (т.к. $\mathbb{R}\backslash V$ - открытое множество). Т.е. нашлась окрестность $B_\varvarepsilon(a)$, которая не пересекается с множеством $V$, а значит $a$ не граничная точка.
    \item[$2) \Rightarrow3)$] \textit{(Если $V$ содержит все свои граничные точки, то оно содежит и все свои предельные)}: \newlineПусть $a$ предельная для $V$ точка и предположим, что $a \not\in V$. Значит $a$ и не граничная точка (т.к. $V$ содержит все свои граничные точки). Поэтому найдётся такое $\varvarepsilon > 0$, что $B_\varvarepsilon(a)\cap V = \o$. Таким образом, $B'_\varvarepsilon(a)\cap V=\o$ и $a$ не предельная для $V$.
    ...
    \item[$4) \Rightarrow1)$] \textit{(Если $a_n\in V$ и $a_n\rightarrow a$, то $a\in V$. А отсюда $V - $ замкнутое множество)}: \newline Пусть $V$ - не замкнуто. $\Leftrightarrow$ $\mathbb{R}\backslash V$ - не открыто $\Rightarrow$ существует такое $a\in \mathbb{R}\backslash V: B_\varvarepsilon(a) \cap V \neq \o$ и при этом $B_\varvarepsilon(a)\not\subset V$. Тогда пусть $\varvarepsilon_n = \frac{1}{n} \Rightarrow \exists a_n\in B_{\frac{1}{n}}(a) \cap V \Rightarrow a_n\in V$ и $a_n\rightarrow a \Rightarrow a\in V$ (по условию). Получили противоречие, а значит $V$ - замкнуто.
\end{itemize}
\end{proof}
...
то найдется такое число $\varvarepsilon>0$, что $(c-\varvarepsilon, c+\varvarepsilon)\subset U_\alpha$.
Т.к. $b_n-a_n = \frac{b-a}{2^n}\to 0$, то $a_n\to c$ и $b_n\to c\ (|c - a_n|\leq |b_n - a_n|; |c - b_n|\leq |b_n - a_n|)$.
Тогда для некоторого номера $n_0$ выполнено $a_{n_0}\in (c-\varvarepsilon, c]$
и $b_{n_0}\in [c, c+\varvarepsilon)$. Т.е.
$[a_{n_0}, b_{n_0}]\subset (c-\varvarepsilon, c+\varvarepsilon)\subset U_\alpha$,
